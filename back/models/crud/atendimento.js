...
class Atendimento {
  ...
  adiciona(item) {
    ...
    const data = `2020-11-20 00:00:00.0`;
    const sql = `INSERT INTO Atendimentos(clienteId, petId, servicoId, data, status, observacoes) VALUES(${clienteId}, ${petId}, ${servicoId}, data='${data}', '${status}', '${observacoes}'); 
  ...
  atualiza(novoItem) {
    ...
    const data = `2020-11-20 00:00:00.0`;
    const sql = `UPDATE Atendimentos SET clienteId=${clienteId}, petId=${petId}, servicoId=${servicoId}, status='${status}', data='${data}', observacoes='${observacoes}' WHERE id=${id}; 
